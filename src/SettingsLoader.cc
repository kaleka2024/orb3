...
/**
 * @brief 模板特化：从 yaml 配置文件读取 float 类型参数
 * @param fSettings OpenCV 文件存储对象，加载 yaml 配置
 * @param name 配置项 key 名称
 * @param found 输出标记，true 代表配置项存在，false 不存在
 * @param required 是否为必填参数，true 缺失直接终止程序；false 缺失给默认值 0.0f
 * @return 读取到的浮点数值
 */
template <>
float SettingsLoader::readParameter<float>(cv::FileStorage& fSettings,
    ...
    // 冗余 return，避免某些编译器路径分析警告
    return 0.0f;
}

/**
 * @brief 模板特化：从 yaml 配置文件读取 int 类型参数
 * @param fSettings OpenCV 文件存储对象，加载 yaml 配置
 * @param name 配置项 key 名称
 * @param found 输出标记，true 代表配置项存在，false 不存在
 * @param required 是否为必填参数，true 缺失直接终止程序；false 缺失给默认值 0
 * @return 读取到的整型数值
 *
 * 修复说明：
 *   原代码在 node.empty() 分支里，required == true 时 exit(-1) 后还有两行死代码，
 *   且 required == false 时没有 else 分支，导致函数末尾没有 return，编译警告：
 *       warning: control reaches end of non-void function [-Wreturn-type]
 *   此处补全 else 分支和末尾 return。
 */
template <>
...
/**
 * @brief 模板特化：从 yaml 配置文件读取 string 字符串参数
 * @param fSettings OpenCV 文件存储对象，加载 yaml 配置
 * @param name 配置项 key 名称
 * @param found 输出标记，true 代表配置项存在，false 不存在
 * @param required 是否为必填参数，true 缺失直接终止程序；false 缺失返回空字符串
 * @return 读取到的字符串
 */
template <>
...
/**
 * @brief 模板特化：从 yaml 配置文件读取 cv::Mat 矩阵参数，用于读取外参矩阵
 * @param fSettings OpenCV 文件存储对象，加载 yaml 配置
 * @param name 配置项 key 名称
 * @param found 输出标记，true 代表配置项存在，false 不存在
 * @param required 是否为必填参数，true 缺失直接终止程序；false 缺失返回空 Mat
 * @return 读取到的 cv::Mat 矩阵
 */
template <>
...
/**
 * @brief 静态入口函数，构造 SettingsLoader 实例并加载配置，对外暴露调用接口
 * @param configFile yaml 配置文件路径
 * @param sensor 传感器类型：单目/双目/IMU-双目/RGBD
 * @param vocabFile ORB 词袋文件路径
 * @return Expected 类型，成功返回 shared_ptr<Settings>，失败返回错误信息
 */
...
/**
 * @brief SettingsLoader 构造函数，内部构造 Settings 智能指针对象
 * @param sensor 传感器类型
 */
...
/**
 * @brief 核心加载函数：读取 yaml 配置，逐项解析所有参数，组装 Settings 对象
 * @param configFile yaml 配置文件路径
 * @param vocabFile ORB 词袋文件路径，可以外部传入覆盖配置文件内部词袋路径
 * @return Expected 类型，成功返回 settings_，失败返回错误字符串
 */
SettingsLoader::Expected SettingsLoader::load(const std::string& configFile,
                                             const std::string& vocabFile) {
    // 以只读模式打开 yaml 配置文件
    ...
    // 如果外部传入词袋文件路径不为空，覆盖 settings 内部词袋路径
    if (vocabFile.size() > 0) {
    ...
    // 执行配置完整性校验，校验全部通过返回 settings 智能指针，否则返回错误
...
/**
 * @brief 读取主相机 Camera1 的标定参数，支持 PinHole / Rectified / KannalaBrandt8 三种模型
 * @param fSettings 已经打开的 yaml 文件存储对象
 */
void SettingsLoader::readCamera1(cv::FileStorage& fSettings) {
    ...
    if (cameraModel == "PinHole") {
        ...
        // 先调用 setMonoCamera 设置相机模型，后续填入畸变系数
        settings_->setMonoCamera(Settings::PinHole, vCalibration);

        // 读取畸变系数 k1，判断配置文件是否存在 k1，存在说明有畸变
        readParameter<float>(fSettings, "Camera1.k1", found, false);
        if (found) {
            // 判断是否存在 k3，OpenCV 畸变系数：有 k3 是 5 参数，无 k3 是 4 参数
        ...
        // 再次调用 setMonoCamera，传入畸变系数，内部标记是否需要图像去畸变
        ...
        // Kannala-Brandt8 鱼眼相机模型，一共 8 个参数 fx fy cx cy k1 k2 k3 k4
        float fx = readParameter<float>(fSettings, "Camera1.fx", found);
        float fy = readParameter<float>(fSettings, "Camera1.fy", found);
        float cx = readParameter<float>(fSettings, "Camera1.cx", found);
        float cy = readParameter<float>(fSettings, "Camera1.cy", found);
...
/**
 * @brief 读取双目系统的 Camera2 右相机标定参数、双目外参、深度阈值
 * @param fSettings 已经打开的 yaml 文件存储对象
 *
 * 修复说明：
 *   原代码在 KannalaBrandt 分支里读取的是 Camera1.k1~k4，这是逻辑 bug，
 *   应该是 Camera2.k1~k4，此处修正。
 */
void SettingsLoader::readCamera2(cv::FileStorage& fSettings) {
    bool found;
    vector<float> vCalibration, vDistortion;
    if (settings_->cameraType_ == Settings::PinHole) {
        // 针孔双目，读取右相机 fx fy cx cy
        ...
        // ================================================================
        // 修复点：原代码读的是 Camera1.k1~k4，应为 Camera2.k1~k4
        // ================================================================
...
/**
 * @brief 读取图像尺寸配置：原始图像宽高、可选缩放后的 newWidth/newHeight、RGB 标记
 * @param fSettings 已经打开的 yaml 文件存储对象
 */
...
void SettingsLoader::readIMU(cv::FileStorage& fSettings) {
    ...
    // 读取 T_b_c1：IMU 本体坐标系到相机 1 坐标系外参矩阵，转换为 Sophus SE3 格式
    cv::Mat cvTbc = readParameter<cv::Mat>(fSettings, "IMU.T_b_c1", found);
    settings_->Tbc_ = Converter::toSophus(cvTbc);

    // 读取可选参数 IMU.InsertKFsWhenLost，跟踪丢失时是否强制插入关键帧，默认 true
...
/**
 * @brief 读取 RGBD 传感器配置：深度缩放因子、深度阈值、基线、bf 参数
 * @param fSettings 已经打开的 yaml 文件存储对象
 */

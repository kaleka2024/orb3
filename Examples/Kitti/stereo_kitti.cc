...
#include <System.h>                // ORB-SLAM3 系统主头文件，SystemFactory 工厂接口
#include <algorithm>               // STL 算法库，sort 排序函数
#include <chrono>                  // C++ 高精度时钟，用于统计各阶段耗时
#include <fstream>                 // 文件读写流，读取 KITTI times.txt 时间戳
#include <iomanip>                 // IO 格式控制，setfill、setw 实现文件名 6 位补零
#include <iostream>                // cout/cerr 标准输入输出
#include <opencv2/core/core.hpp>   // OpenCV 核心模块，Mat 图像容器

using namespace std;

/**
 * @brief 加载 KITTI 双目数据集：左右图像路径、时间戳
 * @param strPathToSequence KITTI 数据集序列根目录
...
/**
 * @brief STEREO 双目模式运行 KITTI 数据集主函数入口
 * @param argc 命令行参数个数
 * @param argv 命令行参数数组
 * @return int 程序退出码，0 正常，非 0 异常
 */
int main(int argc, char **argv) {
    // 校验命令行参数，需要 3 个入参：词袋路径、yaml 配置、数据集序列路径
    if (argc != 4) {
        cerr << endl
    ...
    // process frames. 创建 SLAM 实例；传感器 STEREO 双目，true 开启多数据集标记
    auto exSLAM = ORB_SLAM3::SystemFactory::create(
        argv[1], argv[2], ORB_SLAM3::SensorType::STEREO, true);

    // 判断 SLAM 初始化是否失败，打印错误信息退出
    if (!exSLAM) {
        cerr << "Failure to initialize ORBSLAM3: " << exSLAM.error().msg()
             << endl;
        exit(-1);
    }

    auto SLAM = exSLAM.value(); // 获取 SLAM 系统有效智能指针
    ...
    double t_track = 0.f;  // 单帧总跟踪耗时，单位 ms
    double t_resize = 0.f; // 图像缩放耗时，单位 ms
#endif
    ...
    for (int ni = 0; ni < nImages; ni++) {
        ...
        if (imageScale != 1.f) {
            ...
            t_resize = std::chrono::duration_cast<
                           ...
                           .count();
            SLAM->InsertResizeTime(t_resize); // 将缩放耗时送入 SLAM 内部统计
#endif
        }

#ifdef REGISTER_TIMES
        // 记录跟踪开始时间点（放在缩放之后，只统计 TrackStereo 耗时）
        std::chrono::steady_clock::time_point t1 =
            std::chrono::steady_clock::now();
#endif

        // Pass the images to the SLAM system 向 SLAM 送入双目图像，执行双目跟踪
        ...
        // 计算本帧跟踪耗时，单位秒
        // 注意：t1、t2 只在 REGISTER_TIMES 定义，所以这里也需在宏内
#ifdef REGISTER_TIMES
        double ttrack =
        ...
        // 处理速度快于帧间隔则休眠，usleep 单位微秒
        if (ttrack < T) usleep((T - ttrack) * 1e6);
    }

    // Stop all threads 序列处理完毕，关闭 SLAM 全部后台线程
    SLAM->Shutdown();
    ...
    // Save camera trajectory 保存 KITTI 格式完整相机轨迹
...
/**
 * @brief 读取 KITTI 数据集时间戳文件、拼接左右图像完整路径
 * @param strPathToSequence KITTI 序列根目录
...
void LoadImages(const string &strPathToSequence, vector<string> &vstrImageLeft,
    ...
    // 拼接 6 位补零 png 文件名，KITTI 命名格式 000000.png
